<template>
  <div class="user_content_blcok">
    <div class="user_content_blcok_title" style="position: relative;">发票申请
      ...
      <el-form label-width="130px">
        <el-form-item>
          <div class="submit_btn">
            <el-button type="danger" @click="onSubmit('invoice/apply')">提交</el-button>
          </div>
        </el-form-item>
      </el-form>
    </div>
...
export default {
  ...
  methods: {
    ...
    onSubmit(apply_edit) {
      ...
      if (this.common_info.type == 1) { //普通发票
        if (this.common_info.sort == 1) { //个人
          obj = { ...this.common_info, ...this.myself_info, ...{ token, order_id: this.orderid } };
          validref = "myself_ref";
        }
        if (this.common_info.sort == 2) { //单位
          obj = { ...this.common_info, ...this.company_info, ...{ token, order_id: this.orderid } };
          validref = "company_ref";
        }
      }
      if (this.common_info.type == 2) { //专用发票
        obj = { ...this.common_info, ...this.special_info, ...{ token, order_id: this.orderid } };
        validref = "special_ref";
      }
