<template>
  <div class="login">
    <div class="tophead">
      <header>
        <div class="smlogo">
          <el-image src="/pc/sanmulogo.png" fit="cover" @click="$router.push({name:'company_home'})"></el-image>
          <span>企业注册</span>
        </div>
        <div class="tologin">
          已有账号？
    ...
    <div class="centercontent">
      <!-- <el-form :model="ruleForm" :rules="rules" ref="ruleFormDom" class="elformclass">
        ...
        <el-form-item prop="mobile" :show-message="false">
          <el-input placeholder="手机号" v-model.number="ruleForm.mobile"></el-input>
        </el-form-item>
        <el-form-item prop="code" :show-message="false">
          <el-input placeholder="短信验证码" v-model="ruleForm.code" width="160px" class="send_left"></el-input>
        ...
        <el-form-item prop="social_credit" :show-message="false">
          <el-input placeholder="统一社会信用代码" v-model="ruleForm.social_credit"></el-input>
        </el-form-item>
      </el-form>
      <div class="agree">
      ...
      <div class="callsammu">您好，企业注册请直接联系工作人员</div>
      <div class="tocompany_register">
        <i class="iconfont">&#xe62e;</i>
        <span class="font" @click="companyregister">个人注册</span>
      </div>
    </div>
  </div>
</template>
...
export default {
  data() {
    ...
    return {
      ...
      rules: {
        mobile: [{ required: true, message: '请输入手机号', trigger: "blur" }, { pattern: this.$jiaoyan_phone, message: '手机号格式错误', trigger: "blur" },],//1[3456789]\d{9} 1开头，3456789随机一位，数字0-9随机9位
        ...
        name: [{ required: true, message: '请输入短信验证码', trigger: "blur" }],
        social_credit: [{ required: true, message: '请输入短信验证码', trigger: "blur" }],
  ...
  methods: {
    send_sms() {
      this.$refs.ruleFormDom.validateField("mobile", (error) => {
        // console.log(error)
        // 如果手机号码通过/没有错误信息
        if (!error) {
          this.getcode();
    ...
    // 请求验证码
    getcode() {
      this.$post("/company/send_regcode", { mobile: this.ruleForm.mobile }).then(res => {
        if (res.code !== 200) {
    ...
    tologin() {
      this.$router.replace({ name: "company_login" })
    },

    toregister() {
      let propField = ['mobile', 'password', 'checkPass', 'name', 'social_credit', 'code'];
      Promise.all(propField.map(item => {
        ...
        if (dataarr.length > 0) {
          ...
          this.$post(this.$api.companyRegister, { ...this.ruleForm, ...{ token } }).then(res => {
            console.log(res)
            if (res.code == 200) {
