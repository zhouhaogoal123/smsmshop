    ...
    <div class="head_in">
      <div class="top_shop_left">
        ...
        <li v-if="tohomeshow">
          ...
          <router-link to="/company/home" exact class="tohome">企业首页</router-link>
        </li>
        <li v-if="tosmshow">
          <i class="iconfont" style="padding-right:5px;font-weight:700;">&#xe656;</i>
      ...
      <div class="top_shop_right">
        <ul>
          <li v-if="token&&user_info.client&&user_info.client.name">客户经理：
            <span>{{user_info.client.name}} </span>
            <span style="font-weight:700;">{{user_info.client.phone}}</span>
          </li>
          <li v-if="token&&user_info.client&&user_info.client.name">|</li>
          ...
          <li v-if="token" class="myallxiala" @mouseenter="ismyhover = true" @mouseleave="ismyhover = false">
            <div class="item_xiala" @click="touser">
              ...
              <span class="company_name">企业会员</span>
          ...
          <li v-if="token">|</li>
          <li v-if="token" class="item" @click="toOnlineService">在线客服</li>
          <li>|</li>
          <li class="item">
...
export default {
  ...
  watch: {
    ...
    $route: {
      handler(newVal) {
        // console.log(newVal);
        // 首页不加回到企业首页
        newVal.name == "company_home" || newVal.name == "company_index" ? this.tohomeshow = false : this.tohomeshow = true;
        // 首页加上文字轮播。欢迎你
        newVal.name == "company_home" ? this.fontshow = true : this.fontshow = false;
        // 没有登录，企业首页加上回到三目
        (newVal.name == "company_home" || newVal.name == "company_index") && !this.token ? this.tosmshow = true : this.tosmshow = false;
      },
  ...
  methods: {
    touser() {
      let newUrl = this.$router.resolve("/company/user");
      window.open(newUrl.href, "_blank");
    },
    tologin() {
      this.$router.push({ name: "company_login" });
    },
    toregister() {
      this.$router.push({ name: "company_register" });
    },
    tocompany() { //如果登录了，跳转到企业首页，如果没有登录，跳转到企业中转页
      if (this.token) {
        let newUrl = this.$router.resolve({ name: "company_home" });
        window.open(newUrl.href, "_blank");
      } else {
    ...
    logout() {
      this.$post(this.$api.homeLogout, { token: localStorage.getItem("token") }).then((res) => {
        // console.log(res)
        localStorage.clear();
    ...
    toOnlineService() {
      let newUrl = this.$router.resolve({ name: "company_online_service" })
      window.open(newUrl.href, "_blank")
    },
  },
};
